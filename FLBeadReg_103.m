...
% Beijing, China.
    ...
    % For POI transformation
        case 'New transformation'
            ...
            uiwait(helpdlg('Select single-spot-containing regions (fluorescence micrograph) and bead centres (electron micrograph). When dealing with EM image, press any key to start selection once zooming is OK. Press ESC when you finish.','Instruction'));
            POI_O = [];	% Points of interest on original image
            POI_R = [];	% Points of interest on reference image
            for i = 1:256
                figure(1);
                hold on;
                [cropped, rectout] = imcrop;    % Select a spot on original image
                if (isempty(cropped) || isempty(rectout))
                   break;
                end
                [centre_x, centre_y] = gausscentre2d(cropped); % Find centre by Gaussian fitting
                x1 = round(rectout(1)) - 1 + centre_x;
                y1 = round(rectout(2)) - 1 + centre_y;
